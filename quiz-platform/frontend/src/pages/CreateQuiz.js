...
export default function CreateQuiz() {
  ...
  useEffect(() => {
    if (!isEdit) return;
    axios.get('/api/quizzes/admin/list').then(r => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await axios.put('/api/quizzes/' + id, data);
      } else {
    ...
    <div className="container" style={{ maxWidth: 750 }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {questions.map((q, qi) => (
          <div key={qi} style={{ background: '#fff', border: '1px solid #ddd', borderRadius: 6, padding: 18, marginBottom: 14 }}>
            ...
            <label style={{ display: 'block', marginBottom: 8 }}>
              Options — select the correct one using the radio button
            </label>
            ...
            {q.options.map((opt, oi) => (
              <div key={oi} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <input
                  ...
                  onChange={() => updateQ(qi, 'correctOption', oi)}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          <button type="button" className="btn-gray" onClick={addQuestion}>
            + Add Question
          </button>
          <div className="flex-row">
            <button type="button" className="btn-gray" onClick={() => navigate('/admin')}>
              Cancel
            </button>
